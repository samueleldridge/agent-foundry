...
> **Errata (2026-08, branch stranding):** `POST /api/projects` commits the skeleton + starter eval on `foundry/<name>` and then RESTORES the previously checked-out branch — the workspace is never left stranded on the project branch. `GET /api/health` gained `current_branch` so the UI can surface where the operator actually is; the forge launch resolves fresh skeletons that exist only on `foundry/<name>` (the session pre-flight checks the branch out).

The starter eval is scaffolded **server-side by the project-new route**, validated by `load_eval_spec` before anything is committed. In the editor the file deep-links **human-editable**: the config-write route runs the HUMAN write sandbox (`.foundry/` denied; `evals/` allowed), because the eval set is the operator's artifact — docs/60 makes it immutable to the *optimizer*, not to the human who owns it. The meta-agent's own sandbox keeps refusing `evals/` unchanged (§ Eval assistant). The new-project flow's preferred path over hand-filling TODOs is the eval assistant wizard (§ Eval assistant), which drafts the full set for human review.
...
AI-assisted eval authoring (`foundry.studio.eval_assist`): a lightweight **two-step LLM flow** — clarifying questions, then a complete `EvalSpec`-shaped YAML draft — for the operator request "evals need to be defined by the user; leverage an LLM to write them out fully, with human review".

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/evals/assist/questions` | `{project, description, model?}` → 3–5 clarifying questions `[{id, question, why, suggested_answer?}]` about requirements, input shape, expected output shape, edge cases, category balance |
| `POST` | `/api/evals/assist/draft` | `{project, description, answers: [{id, answer}], case_count?=10, model?}` → a COMPLETE draft: `{yaml, validation, cases[], suggested_path, notes[]}`. The draft is validated through the **real EvalSpec loader** (the same shadow-copy validation the config editor uses); on failure the route makes exactly **one** automatic repair round-trip, feeding the loader's issues back to the model |

Design decisions (settled):

- **NOT the meta-agent.** docs/41 makes the eval set the forge's untouchable objective — the optimizer must never author its own target. The assistant is a separate *pre-forge* surface: a plain `ModelBinding` call chain through the provider abstraction (default `openai/gpt-5-mini`, model selectable with the same key-aware picker as the forge form), structured output via the existing `response_format: json_schema` support.
- **The human gate is the guarantee.** Neither route writes a byte to disk. The wizard's review step shows the drafted cases (id / input / expected, jump-to-line into the YAML) and the YAML in the standard server-validated editor; only the explicit "Save eval set" goes through the existing config-write route (`PUT /files/evals/...` — validate → commit) with the human owning the expected values. Regenerate re-drafts with tweaked answers.
- **Deterministic drafts only.** Generated drafts are pinned to `scope: project`, `target: <project>`, exact/numeric scorers, `deterministic: true` + seed. An `llm_judge`/`rubric`/`user` scorer in a generated draft is a validation *error* — the human opts into judges by editing the saved file.
- **Charge/observe like every LLM surface.** Each request runs on its own `Session` and emits `run.started` / `llm.started` / `llm.completed` / `run.completed`, so assistant spend lands in the SQLite mirror with project attribution; the acts mirror as `studio.eval_assist_questions` / `studio.eval_assist_draft` events. Redaction rules apply unchanged. A missing provider key is a **424** with `env_vars` + remedy (same contract as unavailable projects).
- **Sandbox split (human vs optimizer).** This feature is why the config-write route runs the *human* sandbox (`evals/` writable, `.foundry/` denied) while the meta-agent's `PathSandbox` defaults still refuse `evals/` — enforced by `test_human_eval_save_commits_while_meta_agent_sandbox_still_refuses`.
...
- **PathSandbox**: every filesystem-touching route resolves paths through `foundry.security.PathSandbox`: writes only under `projects/`; `src/foundry/` and `catalog/` read-only (catalog gains write only through the promote route, which is the human-gated promotion path, not a file write). The config-write route runs the HUMAN-shaped sandbox — `.foundry/` denied, `evals/` writable (the operator owns the eval set; § Eval assistant) — while the meta-agent's sandbox keeps `evals/` + `.foundry/` denied. Traversal attempts (`..`, absolute paths, symlink escapes) are refused and logged as `studio.sandbox_refused` events.
